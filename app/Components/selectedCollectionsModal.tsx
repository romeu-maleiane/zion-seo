...
interface ModalComponentProps {
    ...
    setSavedSelectedCollections: (value: string[]) => void
    shopId: string
}
...
function SelectedCollectionsModal({ modalOpen, setModalOpen, collections, setNewCollections, savedSelectedCollections, setSavedSelectedCollections, shopId }: ModalComponentProps) {
    ...
    const handleSetCancel = useCallback(() => {
        clearSelection()
        savedSelectedCollections.forEach(id => {
            handleSelectionChange(SelectionType.Single, true, id);
        });
    }, [clearSelection, handleSelectionChange, savedSelectedCollections])

    const handleSaveSelectedResouces = useCallback(() => {
        setSavedSelectedCollections(selectedResources)
        console.log(savedSelectedCollections)
    ...
    return (
        <>
            <Modal id="my-modal" open={modalOpen} onHide={() => { handleSetCancel(); setModalOpen(false) }}>
                ...
                <TitleBar title="Selected collections">
                    <button disabled={savedSelectedCollections.length === selectedResources.length} onClick={() => { handleSaveSelectedResouces(); setModalOpen(false) }} variant="primary" >Save</button>
...
export default SelectedCollectionsModal

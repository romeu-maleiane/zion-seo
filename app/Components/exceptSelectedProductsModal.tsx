...
interface ModalComponentProps {
    ...
    setSavedExceptSelectedProducts: (value: string[]) => void
    shopId: string
}
...
function ExceptProductsModal({ modalOpen, setModalOpen, products, setNewProducts, savedExceptSelectedProducts, setSavedExceptSelectedProducts, shopId }: ModalComponentProps) {
    ...
    const handleSetCancel = useCallback(() => {
        clearSelection()
        savedExceptSelectedProducts.forEach(id => {
            handleSelectionChange(SelectionType.Single, true, id);
        });
    }, [clearSelection, handleSelectionChange, savedExceptSelectedProducts])

    const handleSaveSelectedResouces = useCallback(() => {
        setSavedExceptSelectedProducts(selectedResources)
        console.log(savedExceptSelectedProducts)
    ...
    return (
        <>
            <Modal id="my-modal" open={modalOpen} onHide={() => { handleSetCancel(); setModalOpen(false) }}>
                ...
                <TitleBar title="All products except selected">
                    <button disabled={savedExceptSelectedProducts.length === selectedResources.length} onClick={() => { handleSaveSelectedResouces(); setModalOpen(false) }} variant="primary" >Save</button>
...
export default ExceptProductsModal
